...
logger = logging.getLogger("lefil.scraper.github_trendings")

GITHUB_DEFAULT_URL: typing.Final[str] = "https://github.com"
GITHUB_TRENDING_URL: typing.Final[str] = f"{GITHUB_DEFAULT_URL}/explore"


class GitHubTrendingsScraper(BaseScraper):
    """
    Scraper les dépôts GitHub en trending.
    ...
    def __init__(self, config: dict | None = None) -> None:
        ...
        self._client = httpx.Client(
            base_url=GITHUB_TRENDING_URL,
            timeout=15.0,
        )

    def _fetch(self) -> Iterator[Entry]:
        try:
            response = self._client.get(GITHUB_TRENDING_URL)
            parser = etree.HTMLParser()
            html_root = etree.fromstring(response.content, parser)
            all_articles = html_root.xpath("//article")
